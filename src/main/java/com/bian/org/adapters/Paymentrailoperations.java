...
@Component
public class Paymentrailoperations implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private PaymentRailOperationsRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Paymentrailoperations.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Paymentrailoperations.class.getName());
         }else{
