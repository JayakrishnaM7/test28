...
@Component
public class Fraudevalution implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private FraudEvalutionRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Fraudevalution.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Fraudevalution.class.getName());
         }else{
