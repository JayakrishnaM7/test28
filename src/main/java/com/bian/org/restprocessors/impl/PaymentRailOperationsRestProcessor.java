...
@Service
public class PaymentRailOperationsRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(PaymentRailOperationsRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.paymentrailoperations.hosturl}")
	private String url;
	
	@Value("${spring.api.paymentrailoperations.context-path}")
	private String contextPath;

	@Value("${spring.api.paymentrailoperations.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  PaymentRailOperationsRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ PaymentRailOperationsRestProcessor.class.getName());
