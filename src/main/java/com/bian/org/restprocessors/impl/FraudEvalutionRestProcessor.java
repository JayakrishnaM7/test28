...
@Service
public class FraudEvalutionRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(FraudEvalutionRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.fraudevalution.hosturl}")
	private String url;
	
	@Value("${spring.api.fraudevalution.context-path}")
	private String contextPath;

	@Value("${spring.api.fraudevalution.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  FraudEvalutionRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ FraudEvalutionRestProcessor.class.getName());
