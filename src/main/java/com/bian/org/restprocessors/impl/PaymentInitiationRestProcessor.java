...
@Service
public class PaymentInitiationRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(PaymentInitiationRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.paymentinitiation.hosturl}")
	private String url;
	
	@Value("${spring.api.paymentinitiation.context-path}")
	private String contextPath;

	@Value("${spring.api.paymentinitiation.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  PaymentInitiationRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ PaymentInitiationRestProcessor.class.getName());
